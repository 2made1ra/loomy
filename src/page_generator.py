...
def generate_page(from_path: str, template_path: str, dest_path: str) -> None:
    ...
    gen_html = template.replace("{{ Title }}", title).replace("{{ Content }}", html_md)
...
def generate_pages_recursive(dir_path_content, template_path, dest_dir_path) -> None:
    ...
    for entry in all_entrys:
        ...
        if os.path.isfile(path_to_entry):
            ...
            generate_page(path_to_entry, template_path, new_path)
        else:
            new_dest_dir = os.path.join(dest_dir_path, entry)
            generate_pages_recursive(path_to_entry, template_path, new_dest_dir)
